...
#definition("线性组合(Linear combination)")[
...
#theorem("张成的空间是最小包含子空间")[一个向量组张成的空间就是包含这些向量的最小的子空间。]
...
#definition("张成(spans)")[
如果$"span"(v_1,v_2,...,v_n) = V$，那么称$v_1,v_2,dots,v_n$张成$V$.
]
#definition("有限维向量空间(finite-dimensional vector space)")[
如果某个空间可以被有限个向量张成，那么这个空间就是一个*有限维向量空间*(finite-dimensional vector space)。
]
#definition("多项式(polynomial)")
[若$FF -> FF$函数$p$可被表示为$p (x) = a_0 + a_1 x + a_2 x^2 + ... + a_n x^n,a_0,a_1,a_2,...,a_n \in FF$，那么这个函数就是一个在$FF$上的*多项式*(polynomial)函数,$a_0,a_1,a_2,...,a_n$称为多项式的*系数*(coefficient)]
#definition("多项式的度(degree)")[
多项式的度(degree)是多项式的最高次幂（最高次幂的次数）。定义$0$的度数为$-infinity$
]
#definition($P(FF),P_m(FF)$)[
$P(FF)$是所有在$FF$上多项式的集合形成的线性空间。容易知道，$P(FF)$是$FF^FF$的子空间

$P_m(FF),m in ZZ^+$指所有在$FF$上次数小于等于$m$的多项式的集合。
]

#definition("无穷维向量空间(infinite-dimensional vector space)")[不是有限维向量空间的向量空间吗，称作无穷维向量空间。]
...
#definition("线性无关(Linearly Independent)")[
...
#definition("基(basis)")[
一个空间$V$的一组*基(basis)*是一组可以张成$V$且线性无关的向量组。
]
#theorem("基的判定定理")[
基的判定定理：$v_1,v_2,dots,v_n$是$V$的一组基 <=> $forall v in V$,存在唯一的$a_1,a_2,dots,a_n in FF$使得$a_1 v_1 + a_2 v_2 + dots + a_n v_n = a$
]
#quote("证明")[
