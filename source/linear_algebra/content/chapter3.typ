...
#definition("线性映射(Linear Mapping)")[ 
...
#definition("零空间(null space)")[

 $T in L(V,W)$,$T$的零空间就是$V$的一个子集,使得${v in V : T v = 0}$,记作$"null" T$,也叫做$T$的核空间(kernel space),记作$ker T$
    ]
...
对于一个函数$T : V -> W$,$T$的值域就是$W$的一个子集${T v}$,记作$"range" T$,也叫函数的像空间(image),记作$im T$. 
]
#theorem[
...
#definition([满射(surjective)])[
如果某个映射$T:V->W$的像空间等于$W$,那么称$T$是一个满射. 
]

#theorem([线性代数基本定理])[

$T in L(V,W)$,$dim V$ = $dim ker T + dim im T$
]
...
设$A$是$n times k$矩阵,$B$是$k times m$矩阵,定义运算$(A B)_"i,j" = sum_"k=1"^k A_"i,k" B_"k,j"$,更加直观的,就是选取$A$的第$i$行和$B$的第$j$列,按元素依次乘在一起再求和,表示新矩阵第$i$行$j$列的元素. 
]
...
如果某矩阵（映射）可逆,那么其逆是唯一的. 

映射$V$可逆$<=>$映射$V$是单射满射（一一对应）
]

对于存在可逆映射的两个空间,他们也有一些潜在的关系,下面加以定义. 

#definition([同构(isomorphism)])[
...
#definition([矩阵的向量表示(matrix of a vector)])[
...
#definition([线性泛函(linear functional)])[
...
- $T in L(V,W)$
  - $V,W$有限维,那么$dim ker T' = dim ker T + dim W - dim V$
  - $ker T = (im T)^0$
- $T$是满的当且仅当$T'$是单的.
]
